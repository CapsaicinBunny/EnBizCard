...
- [app/utils/manifest.ts](app/utils/manifest.ts) — builds the `card.json` written into every export: the card's state flattened to plain data, with assets recorded as paths into the zip rather than embedded. Format version 1.
- [app/utils/import.ts](app/utils/import.ts) — reads a downloaded card (a `.zip` or the unzipped folder) back into the editor, from `card.json` alone. Nothing is scraped out of `index.html`, which is what lets Preview.vue keep changing shape. A manifest is untrusted input: every field is validated, and a bad one costs that field, not the import.
...
- **`npm run lint` is clean, and it is meant to stay that way.** `correctness`, `suspicious` and `perf` are all **errors**, and a curated set of `pedantic`/`restriction` rules is enabled on top — see [.oxlintrc.json](.oxlintrc.json), where every adopted rule and every rejected one carries its reason inline. Nothing is demoted and nothing is suppressed file-by-file. A new finding is a real finding: fix it, or argue the rule down in the config where the next person can see the argument.
- **`style` is deliberately not enabled** — ~1000 findings of formatting opinion that oxfmt already owns. Whole categories are not switched on wholesale, because `pedantic` and `restriction` both mix genuine bug-catchers with rules that are actively wrong here (banning `async`/optional chaining, `getElementById`, the non-null assertions the export scripts depend on).
- **Do not run `oxlint --fix` unattended.** It has produced two real regressions in this repo, both in code whose intent is invisible to the linter: it rewrote `typeof QRCode === 'undefined'` to `QRCode === undefined` in `main.ts` (a `ReferenceError` in exported cards when `qrcode.min.js` is missing — the exact failure that guard exists to survive), and `--fix-suggestions` deleted an `import.meta.dev`-gated `console.warn`, leaving an empty `if`. Read every hunk it writes.
...
- **Icons carry their own terms, and those terms ship with every exported card.** The brand set in `app/assets/icons/` comes from [SVG Stack](https://svgstack.com) under CC BY 4.0 — attribution required, unlike a public-domain set. Adding an icon from somewhere else means checking its licence and updating the credits table in [README.md](README.md), because the file gets inlined into other people's published pages.
